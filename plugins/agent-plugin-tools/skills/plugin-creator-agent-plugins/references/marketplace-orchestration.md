...
The `init` and `add` commands are the low-level interface for a standalone Marketplace whose root `config.json` is human-owned. Do not use them when an orchestrator-owned configuration is the source of truth; use that orchestrator's configuration and synchronization entrypoints instead. Read [management boundaries](management-boundaries.md) to recognize existing managers and handle unavailable tools. The external-definition examples below are for implementing an orchestrator or an established private workflow, not a fallback around an existing owner.
