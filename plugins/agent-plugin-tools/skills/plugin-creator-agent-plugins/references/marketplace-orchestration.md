...
The `init` and `add` commands are the low-level interface for a standalone Marketplace whose root `config.json` is human-owned. Do not use them when an orchestrator-owned configuration is the source of truth; use that orchestrator's configuration and `sync` commands instead. For ai-dotfiles targets, including a shared reference marked `managedBy: "ai-dotfiles/agent-dev"`, read [ai-dotfiles CLI integration](ai-dotfiles-cli.md) for concrete commands and the missing-tool boundary. The external-definition examples below are for implementing an orchestrator or an established private workflow, not a fallback around an existing owner.
...
If an orchestrator supports complete single-owner management, scoped multi-contributor management, and read-only consumption, make every transition explicit. For an ordinary configuration change that narrows a complete definition to contributor or consumer scope, require a successful full `check` so every configured source, package copy, catalog, schema, state, and reference is synchronized. If shared changes block that path, use the owner's explicit review workflow when it provides one: validate the current shared reference, review the assignments to retain, and preserve unselected shared entries. Accepting assignments or narrowing responsibility is not evidence that local source contents are current. Do not switch modes automatically or edit state to bypass a failed check.

Before expanding a contributor or consumer definition into the complete source of truth, require locally resolvable sources for every plugin and Skill already present in the shared Marketplace. A consumer may list and install published content but must not invoke development synchronization. Refuse a transition when its evidence or authority is incomplete rather than inferring ownership from generated copies.
