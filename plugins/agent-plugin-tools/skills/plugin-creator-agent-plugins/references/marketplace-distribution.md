...
Consumer machines follow [Codex integration](codex-integration.md) to register the assembled root and install individual plugins; they do not use the developer manager to install plugins from generated copies. For standalone Skills, use an installer that verifies this custom catalog and protects unmanaged or locally changed destinations. This plugin does not bundle a consumer Skill installer; use the existing management workflow, with [management boundaries](management-boundaries.md) when applicable. The portable Agent Skills specification does not standardize the catalog or its installation commands.
