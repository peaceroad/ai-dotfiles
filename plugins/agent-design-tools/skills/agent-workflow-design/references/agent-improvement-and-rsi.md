...
4. Before classifying the finding, inspect the relevant candidate or prior decision record and its referenced evidence when the user identifies it, when a known record may have reached its revisit condition, or when another read trigger defined by an authorized record store is met. If the finding would otherwise be written as a new unresolved candidate in an existing authorized store, apply the prospective-write trigger in `state-evidence-and-recovery.md` and inspect only the same owner's relevant candidate set. Reclassify any retrieved record from the current evidence; do not scan unrelated candidate records or all decision records, treat a record's existence as evidence, or apply it as an accepted rule.
5. Classify the finding as a reusable change, behavior already covered by an existing rule or route, an unresolved candidate, or a case to reject. Merge semantically equivalent unresolved findings under the same owner instead of accumulating near-duplicate candidates, and do not convert every correction or preferred wording into a durable rule.

   Before classifying a finding as behavior already covered by an existing rule or route, do not stop at confirming that similar guidance exists in the current artifact. As the available evidence and materiality permit, check whether the relevant version was in effect when the issue occurred, whether the actual trigger and reference path could reach it, whether it was specific enough to decide the case, whether an applicable review or completion gate could detect the problem, and what was present in the artifact, intermediate output, or diff while the problem remained.

   Separate verified facts, reasonable inference, and unresolved uncertainty. If the relevant version, rule, route, and verification coverage were adequate, do not add a duplicate rule merely because one execution failed. If the trigger, route, specificity, or verification gate was weak, correct that layer. Do not attribute the failure to a one-off execution miss when material parts of the functional path remain unverified, and do not persist the uncertainty automatically without applying the persistence decision.

   If one case is sufficient to decide, make or propose the smallest authorized correction. If another independent case could materially change the decision, keep the candidate unresolved in the current session first and apply the persistence decision in `state-evidence-and-recovery.md`; do not create a generic candidate store by default. If the problem or decision pattern represented by a persisted candidate is independently observed in another case and that observation could change formalization, scope, exceptions, or the revisit condition, add only the minimum evidence reference needed to recheck it. Treat recurrence as inspectable observations, not a bare counter: do not treat the candidate itself as the observed event, increment a count without inspectable evidence, retain observations that would not change the decision, or use frequency alone to formalize the candidate or strengthen enforcement.
