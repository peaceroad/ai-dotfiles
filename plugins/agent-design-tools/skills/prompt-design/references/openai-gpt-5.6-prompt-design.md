...
The GPT-5.6 guide favors lean instructions and notes more concise default output than GPT-5.5. Use these tendencies to investigate a concrete mismatch:
...
For API work, verify the exact target against the GPT-5.6 model guide and its model page, such as GPT-5.6 Sol. As checked above, GPT-5.6 supports `none`, `low`, `medium`, `high`, `xhigh`, and `max` effort, with `medium` the default. Pro mode and effort are separate controls; requesting Pro in prose does not configure it.
...
- [GPT-5.6 model guide](https://developers.openai.com/api/docs/guides/latest-model?model=gpt-5.6), including [prompting best practices](https://developers.openai.com/api/docs/guides/latest-model?model=gpt-5.6#prompting-best-practices)
