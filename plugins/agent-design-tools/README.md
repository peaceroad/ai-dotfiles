...
`prompt-gemini-reference` reads the shared `prompt-design` entry point and the Gemini foundation, adding Gem or media references only when needed. A Gemini target does not load Astra guidance merely because Codex is designing the prompt. Requests for image/video prompt text do not authorize generation; when execution is also requested, the chosen Gemini surface needs its own available workflow. Neither this skill nor this plugin supplies that access or silently substitutes another provider.
