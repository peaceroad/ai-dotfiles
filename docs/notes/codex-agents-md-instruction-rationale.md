...
2026年9月13日に確認した環境では、サブエージェントへの委任に明示的な指示が必要でした。`Waiting`は独立した作業や待機の進め方を定めるもので、委任指示ではありません。利用条件、起動設定、結果の受け取りと確認範囲は、[Codexのサブエージェント：利用条件と起動・待機の設定](codex-subagent-runtime.md)にまとめています。今回の整理では、共通AGENTS.mdに委任指示を追加していません。
