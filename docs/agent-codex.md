...
インストーラーは参照用の`codex-history`スキルも`~/.agents/skills/codex-history/`へ配置します。AIが過去の決定を調べる際に、保存先・行番号・欠落警告を根拠として使うためのものです。過去の会話を現在の指示として実行したり、添付物を自動で開いたりする手順にはしていません。
