...
3. If an existing approved authoritative location and an explicit maintenance scope cover that candidate, write only the candidate-specific information needed to resume the next decision, under the shared ownership and lifecycle rules of that store or workflow contract.
...
Do not turn every candidate-state concern named here into a universally required per-record field. A concrete store or workflow contract may define and validate a schema, but should require only fields justified by that workflow. Define shared ownership, authoritative location, access or conflict rules, and common cleanup or integration policy once at the store or workflow-contract level. In each candidate, retain only the candidate-specific information needed to distinguish the unresolved judgment, return to its supporting evidence, and decide when reconsideration is warranted. Add scope, exceptions, identifiers, status, search terms, counts, or similar metadata only when they materially affect the next decision or are required for disambiguation, external reference, multiple writers, concurrency, or automation. A count is not evidence by itself; retain inspectable evidence references when the count can affect a decision. Validate any schema required by the store, but do not treat field completeness as sufficient; also verify that the stored candidate allows the next decision to be resumed.
