...
このノートは、`~/.codex/AGENTS.md`にある指示について、英語の指示文、日本語訳、設計理由、確認時点のシステム／開発者指示との関係、運用上の補足を整理したものです。既存の指示は2026年8月11日に整理し、`Waiting`は2026年9月1日、`Browser`は2026年9月2日に追加しました。
...
このセクションは、内蔵Browserと外部ブラウザの選択を固定せず、Codexが外部ブラウザを使う場合だけChromeを優先します。
...
> 外部ブラウザを使用する場合は、私がブラウザを指定しない限りChromeを優先します。

### 背景と理由

`When using an external browser`で適用範囲を限定し、内蔵Browserより外部ブラウザを優先する意味にならないようにしています。`prefer Chrome`は別のブラウザへの切り替えを禁止せず、`unless I specify a browser`は現在の依頼で指定されたブラウザを優先します。

### 現在の上位指示との関係

2026年9月2日時点の上位指示には、EdgeをChromeより優先する規則はありません。Browser操作にはブラウザを指定する経路と、選択を実行環境に委ねる経路があるため、この指示は外部ブラウザが未指定の場合の優先順位だけを補います。ツールの選択や安全確認に関する上位指示は変更しません。

### 補足

`config.toml`ではなく`AGENTS.md`へ置く理由は、[Codex Browserの設定と安全性](./codex-browser-config-and-security.md#ブラウザの選択とagentsmd)で説明しています。`~/.codex/AGENTS.md`内では、共通のツール選択方針として`Waiting`の直後、Windows固有のファイル規則より前に配置します。見出しの順序は指示の優先順位に影響しません。
